...
class GeneralPurposeAgent:
    def __init__(self, company):
        ...
        self.vectorstore = Chroma(persist_directory=self.chroma_dir, embedding_function=embeddings)
        self.retriever = self.vectorstore.as_retriever()
        self.prompt = ChatPromptTemplate.from_template("""You are a general purpose support agent. Use all available company knowledge base files to answer the user's question.\n{context}\n\nQuestion: {question}""")
    ...
    def handle_query(self, query, context=None):
        try:
            ...
            print(f"GeneralPurposeAgent error: {e}")
